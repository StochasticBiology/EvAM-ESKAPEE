...
for(bug in ESKAPEE) {
  ...
  if(nrow(this.df) > max.row) {
    this.df.sub = this.df[sample(1:nrow(this.df), max.row),]
  } else {
    this.df.sub = this.df
  }
  fit[[bug]] = hyperinf(this.df.sub, reversible=TRUE)
  #fit[[bug]] = hyperinf(this.df)
  fit.plot[[bug]] = plot_hyperinf(fit[[bug]]) + theme(legend.position="none")
}
save(fit, file="eskape-rev.Rdata")


# this is taking about an hour maybe, with max.row = 100
# could parallelise over bugs of course


set.seed(1)
fit.ir = fit.plot.ir = list()
max.row = 100
for(bug in ESKAPEE) {
  this.df = final_df[final_df$eskapee == bug,c(1,3:ncol(final_df))]
  if(nrow(this.df) > max.row) {
  ...
  fit.ir[[bug]] = hyperinf(this.df.sub)
  #fit[[bug]] = hyperinf(this.df)
  fit.plot.ir[[bug]] = plot_hyperinf(fit.ir[[bug]]) + theme(legend.position="none")
}

fit.ir.full = fit.plot.ir.full = list()
for(bug in ESKAPEE) {
  this.df = final_df[final_df$eskapee == bug,c(1,3:ncol(final_df))]
    this.df.sub = this.df
  fit.ir.full[[bug]] = hyperinf(this.df.sub)
  #fit[[bug]] = hyperinf(this.df)
  fit.plot.ir.full[[bug]] = plot_hyperinf(fit.ir.full[[bug]]) + theme(legend.position="none")
}

sf = 2
png("eskape-rev.png", width=700*sf, height=200*sf, res=72*sf)
ggarrange(plot_hyperinf_bubbles(fit, expt.names = ESKAPEE,
                      feature.names = substr(drug.labels, start=1, stop=3)) + 
            theme(legend.position="none"),
          plot_hyperinf_bubbles(fit.ir, expt.names = ESKAPEE,
                                feature.names = substr(drug.labels, start=1, stop=3)) + 
            theme(legend.position="none"),
          plot_hyperinf_bubbles(fit.ir.full, expt.names = ESKAPEE,
                                feature.names = substr(drug.labels, start=1, stop=3)),
          labels=c("HyperMk", "HyperHMM", "HyperHMM-full"),
          widths=c(1,1,1.75),
          nrow=1)
dev.off()

ggarrange(plotlist=fit.plot)
...
plot_hyperinf_compare_orderings(fit[[1]], fit[[2]], threshold=0.2)
...
ht.fits = ht.int.graphs = list()
for(bug in ESKAPEE) {
  l2rep = full_to_squared(fit[[bug]])
  trial = l2rep[1,]
  this.m = as.matrix(final_df[final_df$eskapee == bug,3:ncol(final_df)])
  ht.fit[[bug]] = HyperTraPS(this.m, initialparams=trial, 
                             length=3, kernel=4, penalty=1)
  ht.fit[[bug]]$featurenames = drug.labels
  ht.int.graphs[[bug]] = plotHypercube.influencegraph(ht.fit[[bug]], 
                                                      cv.thresh = 0.5)
}
for(bug in ESKAPEE) {
  ht.int.graphs[[bug]] = plotHypercube.influencegraph(ht.fit[[bug]], 
...
#plotHypercube.lik.trace(ht.fit[1])
#plotHypercube.influencegraph(ht.fit, cv.thresh = 0.5)


###########

# don't trust these uncorrected interactions -- overfitting likely from HyperHMM
