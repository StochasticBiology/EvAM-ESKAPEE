...
plot_hyperinf_bubbles(list(fit[[1]], fit.phy[[1]]), expt.names = c("CS", "Phy"),
                          feature.names = substr(drug.labels, start=1, stop=3)) 

plot_hyperinf_ordering_matrices(list(fit[[1]], fit.phy[[1]], fit.phy[[1]]), expt.names = c("CS", "Phy1", "Phy2")) +
  theme(axis.text.x = element_text(angle=45, hjust=1))
        
m1 = ordering_matrix(fit[[1]])
m2 = ordering_matrix(fit.phy[[1]])
...
png(paste0("eskapee-expt-", expt, "-compare-boot.png", collapse=""),
    width=700*sf, height=300*sf, res=72*sf)
print(
  ggarrange(
    plot_hyperinf_comparative(fit, expt.names = ESKAPEE,
                              feature.names = substr(drug.labels, start=1, stop=3),
    ...
    ,
    plot_hyperinf_bubbles(fit, expt.names = ESKAPEE, p.scale=0.5,
                          feature.names = gsub("-", "-\n", drug.labels)) + xlab("Step") + scale_x_continuous(breaks = 1:10)
    , widths=c(1,2.5)
  )
)
dev.off()

p = plot_hyperinf_bubbles(fit, expt.names = ESKAPEE,
                          feature.names = gsub("-", "-\n", drug.labels)) + xlab("Step") 
